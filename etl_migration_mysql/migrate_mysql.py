#!/usr/bin/env python3
"""
migrate_mysql.py  —  MSSQL -> MySQL migration tool
===================================================
11-phase pipeline:
...
class Auditor:
    ...
    def _get_fks(self, cur, schema, table):
        ...
        for r in cur.fetchall():
            ...
            # MySQL does not support SET DEFAULT — map to RESTRICT
...
class Loader:
            ...
            while True:
                ...
                for row in rows:
                    ...
                    for i, v in enumerate(row):
                        mysql_type = t["columns"][i]["mysql_type"] if i < len(t["columns"]) else ""
                        if v is None:
                            out.append(r"\N")
                        elif isinstance(v, bool):
                            # Python bool from BIT column -> 1/0 for MySQL TINYINT(1)
    ...
    def _load_table(self, t: dict) -> dict:
        ...
        try:
            ...
            self.log.info("  Extracted %s -> %s", label, os.path.basename(csv_path))

            col_list = ", ".join(qi(c["mysql_name"]) for c in t["columns"])

            # Build INSERT using LOAD DATA LOCAL INFILE equivalent via batch inserts
            # (LOAD DATA INFILE requires server-side file access; batch INSERT is safer)
            self._batch_insert(t, csv_path)
    ...
    def _batch_insert(self, t: dict, csv_path: str):
        """Insert rows in batches of 500 using parameterised INSERT."""
        table    = t["target_table"]
        cols     = t["columns"]
        col_list = ", ".join(qi(c["mysql_name"]) for c in cols)
            ...
            for row in reader:
                ...
                for i, val in enumerate(row):
                    if val == r"\N":
                        ...
                        mtype = cols[i]["mysql_type"].upper()
                        if any(k in mtype for k in ("INT", "DECIMAL", "FLOAT",
                                                     "DOUBLE", "NUMERIC")):
                            try:
                                typed.append(float(val) if "." in val else int(val))
                            except (ValueError, TypeError):
                                typed.append(None if val == "" else val)
                        elif "TINYINT(1)" in mtype:
                            # boolean
                            typed.append(1 if val in ("True", "true", "1") else 0)
                        else:
                            typed.append(val)
                    else:
                        typed.append(val)
                batch_rows.append(typed)
    ...
    def apply_constraints(self):
        ...
        for t in self.audit["tables"]:
            ...
            for fk in t["foreign_keys"]:
                # MySQL max identifier length is 64 chars — truncate if needed
                fk_name = fk["name"][:64]
                sql = (
    ...
    def _translate_check(clause: str) -> str | None:
        ...
        # MSSQL UPPER() -> MySQL UPPER() (compatible)
        # MSSQL YEAR() -> MySQL YEAR() (compatible)
        # MSSQL DAY() -> MySQL DAY() (compatible)
        return c
    ...
    def create_indexes(self):
        ...
        for t in self.audit["tables"]:
            ...
            for idx in t["indexes"]:
                ...
                # MySQL prefix length needed for TEXT/BLOB columns
                col_parts = []
                ...
                cols = ", ".join(col_parts)
                idx_name = idx["name"][:64]   # MySQL max identifier = 64 chars
                sql = (
...
class ObjectMigrator:
    ...
    # Views that use MSSQL-only features and cannot be auto-migrated
    ...
    def _extract_view_body(definition: str) -> str | None:
        ...
        if not definition:
            return None
        d = definition.strip()
        # Remove SET lines and GO
        d = re.sub(r'(?im)^\s*SET\s+\w+\s+\w+\s*;?\s*$', '', d)
        d = re.sub(r'(?im)^\s*GO\s*$', '', d)
        d = d.strip()

        # Find CREATE VIEW and scan for top-level AS
        ...
        # Convert [bracket] -> `backtick` lowercase
        d = re.sub(r'\[([^\]]+)\]', lambda m: '`' + m.group(1).lower() + '`', d)

        # Fix MSSQL column alias syntax: "alias" = expression  ->  expression AS `alias`
        # e.g.  ,"stateprovincename" = sp.`name`
        # becomes  ,sp.`name` AS `stateprovincename`
        d = re.sub(
            r'(?m),?\s*"([^"]+)"\s*=\s*([^,\n]+)',
    ...
    def _check_unsupported(self, body: str) -> str | None:
        """Return the name of the first unsupported MSSQL feature found, or None."""
    ...
    def _remap_view_schemas(self, body: str) -> str:
        """
        MySQL has no sub-schemas. All `schema`.`table` references must
        become `tgt_db`.`table`.

        Two passes:
          1. Named known schemas -> tgt_db  (catches leftover schema refs)
          2. Generic `word`.`word` pattern  (catches anything else)
        """
        db = self.tgt_db

        # Remove dbo. function calls that can't run in MySQL
        # e.g. dbo.ufnGetContactInformation -> just the function name (will still fail
        # but gives a cleaner error). Flag these views for manual rewrite.
        body = re.sub(r'`dbo`\.`([^`]+)`', r'``', body, flags=re.IGNORECASE)

        # Pass 1: explicit schema names that may appear in view bodies
        ...
        # Pass 2: any remaining `word`.`word` that is schema.table style
        # Only replace if the left side is NOT the tgt_db itself
        body = re.sub(
            rf'`(?!{re.escape(db)}`)([^`]+)`\.(`[^`]+`)',
    ...
    def migrate_views(self):
        ...
        for v in self.audit.get("views", []):
            ...
            # Extract body first (strips SET/GO lines), then check for
            # unsupported features. PIVOT/APPLY can be hidden after GO separators.
            body = self._extract_view_body(raw)

            # Pre-check on BOTH raw and extracted body to catch all cases
            ...
            if unsupported:
                ...
                unsupported_log.append({"view": name, "reason": unsupported})
                skip += 1
                continue
            if not body:
                skip += 1
        ...
        if unsupported_log:
            self.log.info(
                "  %d views need manual rewrite — see view_rewrite_guide.txt",
                len(unsupported_log)
            )
...
def final_report(my_conn, tgt_db: str, audit: dict,
    ...
    for t in audit["tables"]:
        ...
        if match:
            log.info("  [PASS] %s  %d rows", table, pg_rows)
        else:
            log.warning("  [FAIL] %s  src=%d tgt=%d", table, src_rows, pg_rows)
...
def build_parser():
    p = argparse.ArgumentParser(
        ...
        description="MSSQL -> MySQL migration tool",
...
def main():
    ...
    log.info("=" * 60)
    log.info("MSSQL -> MySQL Migration")

...
class ObjectMigrator:
    ...
    def _remap_view_schemas(self, body: str) -> str:
        """
        MySQL has no sub-schemas. Views reference `schema`.`table` which
        must become `tgt_db`.`table`.
        Replace all `schema_name`.`table_name` patterns with `tgt_db`.`table_name`.
        Known MSSQL source schemas in AdventureWorks are replaced generically.
        """
        # After _extract_view_body, identifiers are already backtick-quoted.
        # Pattern: `schemaname`.`tablename` -> `tgt_db`.`tablename`
        db = self.tgt_db
        # Replace any `word`.`word` that looks like schema.table
        body = re.sub(
            r'`([^`]+)`\.(`[^`]+`)',
            lambda m: f'`{db}`.' + m.group(2),
            body
        )
        return body
    ...
    def migrate_views(self):
        ...
        for v in self.audit.get("views", []):
            name = v["name"].lower()
            body = self._extract_view_body(v["definition"] or "")
            if not body:
                skip += 1
                continue
            # Remap schema references to tgt_db
